...
assert.ok(presenter.run('mediaActions()').includes('rename-item'));
assert.ok(!presenter.run('mediaActions()').includes('purge-item'));
presenter.run("globalThis.StageAuth={user:{role:'admin'}}");
assert.ok(presenter.run('mediaActions()').includes('purge-item'));
presenter.run("globalThis.StageAuth={user:{role:'operator'}}");
assert.ok(!presenter.run('mediaActions()').includes('purge-item'));
...
viewer.run(`sync.onState(${JSON.stringify({...state,playback:{...state.playback,blackout:false,paused:true}})})`);
assert.equal(viewer.document.querySelector('#blackout').classList.contains('on'),true,'Normal pause uses the same transmission screen as standby');
assert.equal(viewer.document.querySelector('#progressWrap').style.display,'none','Paused screen hides progress');
